...
class Transform():
    ...
    def get_data_list(self):
        app = Extract()
        return app.get_data_from_bucket() # Loads data from DB, returns "raw_data"
    ...
    def transform_new_data(self, data_list):
        transformed_data_new = [] # Clean list to populate with transformed data
        drink_dict = {} # Clean dictionary to populate with split drink, size and flavour
        for row in data_list:
            t_date, t_time = self.date_breaker(row[0])  # defines variables for split date and time from date breaker
            t_location = row[1] # taken directly from raw_data
            t_first_name, t_last_name = self.person_breaker(row[2]) # splits first name from customer name.
            t_order = row[3] # taken directly from raw_data
            t_drink_menu = self.get_drink_list(self.drink_breaker(row[3]), drink_dict)
            # t_brok_flavour = self.flavour_breaker(self.drink_breaker(row[4]), drink_dict)
            t_price = int(float(row[4])*100)
            t_method = self.pay_method(row[5])
            # t_card = self.card_masker(row[7])
            transformed_data_new.append([t_date, t_time, t_location, t_first_name, t_last_name, t_order, t_price, t_method])
        print(transformed_data_new)
        return (transformed_data_new, drink_dict)
...
# app = Transform()
# app.transform_new_data(app.get_data_list())